...
def connect():
    conn=sqlite3.connect("bookdesk.db")
    cur=conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS library1 (id INTEGER PRIMARY KEY, title TEXT, author TEXT, year INTEGER, type TEXT, publisher TEXT, artist TEXT)")
    conn.commit()
    conn.close()
...
def insert(title,author,year,type,publisher,artist):
    conn=sqlite3.connect("bookdesk.db")
    cur=conn.cursor()
    cur.execute("INSERT INTO library1 VALUES (NULL,?,?,?,?,?,?)", (title, author, year, type, publisher, artist))
    conn.commit()
    conn.close()
    view()
...
def search(title="",author="",year="",type="",publisher="",artist=""):
    conn=sqlite3.connect("bookdesk.db")
    cur=conn.cursor()
    cur.execute("SELECT * FROM library1 WHERE title=? OR author=? OR year=? OR type=? OR publisher=? OR artist=?", (title, author, year, type, publisher, artist))
    rows=cur.fetchall()
    conn.close()
    return rows
...
def update(id,title,author,year,type,publisher,artist,quality,price,location,genre,weight,pages,isbn,description,image,notes):
    conn=sqlite3.connect("bookdesk.db")
    cur=conn.cursor()
    cur.execute("UPDATE library1 SET title=?, author=?, year=?, type=?, publisher=?, artist=? WHERE id=?", (title, author, year, type, publisher, artist, id))
    conn.commit()
    conn.close()
...
# insert("Hello", "Bob Dylan", 1940, "Hardcover", "Random House", "Justin Nash")
